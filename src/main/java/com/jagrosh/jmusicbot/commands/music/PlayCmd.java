...
public class PlayCmd extends MusicCommand
{
    ...
    public void doCommand(CommandEvent event) 
    {
        if(event.getArgs().isEmpty() && event.getMessage().getAttachments().isEmpty())
        {
            ...
            var builder = new StringBuilder(event.getClient().getWarning()+" Play Commands:\n");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <song title>` - plays the first result from Youtube");
            builder.append("\n`").append(event.getClient().getPrefix()).append(name).append(" <URL>` - plays the provided song, playlist, or stream");
            for(var cmd: children)
    ...
    private final class ResultHandler implements AudioLoadResultHandler
    {
        ...
        private void loadSingle(AudioTrack track, AudioPlaylist playlist)
        {
            ...
            var handler = (AudioHandler)event.getGuild().getAudioManager().getSendingHandler();
            var pos = handler.addTrack(new QueuedTrack(track, RequestMetadata.fromResultHandler(track, event)))+1;
        ...
        public void playlistLoaded(AudioPlaylist playlist)
        {
            ...
            {
                var single = playlist.getSelectedTrack()==null ? playlist.getTracks().get(0) : playlist.getSelectedTrack();
                loadSingle(single, null);
            }
            else if (playlist.getSelectedTrack()!=null)
            {
                var single = playlist.getSelectedTrack();
                loadSingle(single, playlist);
            }
            else
            {
                var count = loadPlaylist(playlist, null);
    ...
    public class PlaylistCmd extends MusicCommand
    {
        ...
        public void doCommand(CommandEvent event) 
        {
            ...
            var playlist = bot.getPlaylistLoader().getPlaylist(event.getArgs());
            if(playlist==null)
            {
            ...
            event.getChannel().sendMessage(loadingEmoji+" Loading playlist **"+event.getArgs()+"**... ("+playlist.getItems().size()+" items)").queue(m -> 
            {
                ...
                playlist.loadTracks(bot.getPlayerManager(), (at)->handler.addTrack(new QueuedTrack(at, RequestMetadata.fromResultHandler(at, event))), () -> {
                    var builder = new StringBuilder(playlist.getTracks().isEmpty()
